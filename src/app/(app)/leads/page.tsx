...
import type { Lead, LeadStatus, FilmingStatus } from "@/lib/types";

export default async function LeadsPage({
  searchParams,
}: {
  searchParams: Promise<{ status?: string; filming?: string }>;
}) {
  const profile = await requireProfile();
  const { status, filming } = await searchParams;
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-xl font-semibold text-neutral-900">Mine leads</h1>
        <p className="text-sm text-neutral-500">
          Alle leads som er eller har vært tildelt deg.
        </p>
      </div>
      ...
      <Card>
        <CardContent className="pt-5">
          <form method="get" className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-neutral-500">Filming</label>
